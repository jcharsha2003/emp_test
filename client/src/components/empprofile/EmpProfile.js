...
const EmpProfile = () => {
  ...
  const getUsers = () => {
    axios
      .get(`https://emp-test.onrender.com/user-api/get-emp/${currentUser.email}`, {
  ...
  const saveModifiedUser = () => {
    ...
    if(Object.keys(errors).length===0){
      ...
      axios
        .put(`https://emp-test.onrender.com/user-api/update-user`, modifieduser, {
          headers: { Authorization: "Bearer " + token },
        })
        .then((response) => {
          if (response.status === 200) {
